//
// Created by yc chow on 2021/3/20.
//
...
//DEFINE_string(localaddr, ":30011", "local listen address");
//DEFINE_string(remoteaddr, ":30010", "kcp server address");
...
//
//DEFINE_string(price, "0.001", "transaction fee per MiB");
//DEFINE_string(beneficiary_addr, "", "remote nkn beneficiary addr");
//DEFINE_string(pubkey, "", "remote pubkey");
//DEFINE_string(seed, "", "local payment account");
//DEFINE_uint32(serviceid, 0, "tuna service id");
...
po::variables_map parse(int ac, char *av[]) {
    try {
        string localaddr;
        string remoteaddr;
        string price;
        string beneficiary_addr;
        string pubkey;
        string seed;
        int serviceid;
        po::options_description desc("Allowed options");
        desc.add_options()
                ("help", "tuna-cpp help message")
                ("localaddr,l", po::value<std::string>()->default_value("127.0.0.1:30090"),
                 "local list addr")
                ("remoteaddr,r", po::value<string>(&remoteaddr)->required(),
                 "remote tuna server addr")
                ("price", po::value<string>(&price)->default_value("0.001"),
                 "remote tuna service price per MB")
                ("beneficiary,b", po::value<string>(&beneficiary_addr),
                 "remote tuna server beneficiary addr")
                ("pubkey", po::value<string>(&pubkey),
                 "remote tuna server addr")
                ("seed", po::value<string>(&seed),
                 "local wallet seed")
                ("serviceid", po::value<int>(&serviceid),
                 "remote tuna service id");
        ...
        if (vm.count("localaddr")) {
            cout << "localaddr: "
                 << vm["localaddr"].as<string>() << "\n";
        }

        if (vm.count("remoteaddr")) {
            cout << "remoteaddr: "
                 << vm["remoteaddr"].as<string>() << "\n";
        }
        ...
        if (vm.count("beneficiary")) {
            cout << "beneficiary addr: "
                 << vm["beneficiary"].as<string>() << "\n";
        }
